...
<Callout variant="info">This authentication is only related to the `/auction` endpoints.</Callout>
...
<EndpointDetails method="POST" path="/auction/auth/register" />
...
<EndpointDetails method="POST" path="/auction/auth/login" />
