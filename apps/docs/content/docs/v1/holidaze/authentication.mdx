...
<Callout variant="info">This authentication is only related to the `/holidaze` endpoints.</Callout>
...
<EndpointDetails method="POST" path="/holidaze/auth/register" />
...
<EndpointDetails method="POST" path="/holidaze/auth/login" />
