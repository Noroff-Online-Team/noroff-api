...
const run = async () => {
  console.log(
    "Updating src/helpers/http-status-codes.ts and src/helpers/http-status-phrases.ts"
  )
  ...
  await project.save()
  await execSync(
    "biome format --write ./src/helpers/http-status-codes.ts ./src/helpers/http-status-phrases.ts"
  )
  console.log(
