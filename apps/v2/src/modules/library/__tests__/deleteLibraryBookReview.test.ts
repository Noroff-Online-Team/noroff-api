...
const THIRD_REVIEW_ID = "93062dec-6b41-4ce8-8586-86bcd1a0288a"
...
beforeEach(async () => {
  ...
  const { name: thirdName, bearerToken: thirdBearerToken } =
    await getAuthCredentials({
      name: "test_user_three",
      email: "test_user_three@noroff.no"
    })

  USER_NAME = name
  SECOND_USER_NAME = secondName
  THIRD_USER_NAME = thirdName
  BEARER_TOKEN = bearerToken
  SECOND_BEARER_TOKEN = secondBearerToken
  THIRD_BEARER_TOKEN = thirdBearerToken
  API_KEY = apiKey
...
describe("[DELETE] /library/:id/reviews/:reviewId", () => {
  it("should return 204 when successfully deleted a review", async () => {
    ...
    const review = await db.libraryBookReview.findUnique({
      where: { id: REVIEW_ID }
    })
