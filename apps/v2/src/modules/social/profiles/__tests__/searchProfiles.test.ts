...
describe("[GET] /social/profiles/search", () => {
  ...
  it("should return empty array if no profiles match query", async () => {
    const response = await server.inject({
      url: `/social/profiles/search?q=random`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
  ...
  it("should return profiles with pagination and sort", async () => {
    const response = await server.inject({
      url: `/social/profiles/search?q=user&sort=name&sortOrder=desc&limit=1&page=1`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
