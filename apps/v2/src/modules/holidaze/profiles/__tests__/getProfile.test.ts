...
describe("[GET] /holidaze/profiles/:id", () => {
  ...
  it("should return single profile with venues", async () => {
    const response = await server.inject({
      url: `/holidaze/profiles/${TEST_USER_NAME}?_venues=true`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
  ...
  it("should return single profile with bookings", async () => {
    const response = await server.inject({
      url: `/holidaze/profiles/${TEST_USER_NAME}?_bookings=true`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
  ...
  it("should throw 404 error when attempting to access profile that does not exist", async () => {
    const response = await server.inject({
      url: `/holidaze/profiles/does_not_exist`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
