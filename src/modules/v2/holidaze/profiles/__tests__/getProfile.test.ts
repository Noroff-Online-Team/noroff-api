...
describe("[GET] /v2/holidaze/profiles/:id", () => {
  it("should return single profile based on name", async () => {
    const response = await server.inject({
      url: `/api/v2/holidaze/profiles/${TEST_USER_NAME}`,
      method: "GET",
      headers: {
  ...
  it("should return single profile with venues", async () => {
    ...
    expect(res.data).toBeDefined()
    expect(res.data.name).toBe("test_user")
    expect(res.data.venues).toBeDefined()
    expect(res.data.venues).toHaveLength(0)
    expect(res.meta).toBeDefined()
    expect(res.meta).toStrictEqual({
      isFirstPage: true,
      isLastPage: true,
  ...
  it("should return single profile with bookings", async () => {
    const response = await server.inject({
      url: `/api/v2/holidaze/profiles/${TEST_USER_NAME}?_bookings=true`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
    ...
    expect(res.data.bookings).toHaveLength(0)
    expect(res.meta).toBeDefined()
    expect(res.meta).toStrictEqual({
      isFirstPage: true,
      isLastPage: true,
  ...
  it("should throw 404 error when attempting to access profile that does not exist", async () => {
    const response = await server.inject({
      url: `/api/v2/holidaze/profiles/does_not_exist`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${BEARER_TOKEN}`,
        "X-Noroff-API-Key": API_KEY
